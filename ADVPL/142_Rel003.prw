...
// Substitui o cabecalho padrao pelo objeto Cabecalho (Objetos.prw).
...
// Cria o objeto Cabeçalho.
oCabec := Cabec():New("Extrato de Contas-Correntes")

cNomeAnt := ""
nSaldo   := 0

dbSelectArea("SZ2")               // Seleciona o arquivo de Transaçoes.
dbOrderNickName("NOME_NR_IT")     // Seleciona a chave "Filial + Nome + Nro.Trans. + Item".
dbGoTop()                         // Vai para o primeiro registro, de acordo com a ordem selecionada.

SetRegua(RecCount())              // Inicializa a regua.

While !SZ2->(Eof())               // Executa a sequencia de comandos enquanto nao for Fim de Arquivo.
   ...
   // A variavel cNomeAnt guarda o nome da Conta que está sendo impressa. A cada registro lido, ve se é diferente
   // do conteudo do campo Z2_Nome.
   // Se for diferente, significa que mudou a Conta. Neste caso, deve-se:
   //    -imprimir uma linha de separação com a Conta anterior;
   //    -imprimir o nome da nova Conta;
   //    -guardar o nome da nova Conta na variavel cNomeAnt;
   //    -inicializar a variavel nSaldo.

   If SZ2->Z2_Nome <> cNomeAnt                        // Mudou a Conta.
      @PRow()+1,000 PSay __PrtThinLine()              // Imprime uma linha para separar a Conta anterior.
      @PRow()+1,000 PSay "Nome: " + SZ2->Z2_Nome      // Imprime o nome da nova Conta.
      @PRow()+1,000 PSay " "                          // Pula uma linha em branco.
      cNomeAnt := SZ2->Z2_Nome                        // Guarda o nome da nova Conta.
   ...
   SZ2->(dbSkip()) // Vai para o proximo registro, tambem pela ordem selecionada.

End

// Imprime uma linha final, após a ultima Conta listado.
@PRow()+1,000 PSay __PrtThinLine()

Return Nil         // Termina a função, retornando ao ponto onde foi chamada. Não retorna nenhum valor.

//----------------------------------------------------------------------------------------------------------------// 
// Rotina de impressao do cabeçalho.
//----------------------------------------------------------------------------------------------------------------// 
Static Function Cabec()

oCabec:Imprime()   // Executa o metodo Imprime() do objeto oCabec.

// Imprime o cabeçalho.
@PRow()+1,000 PSay "  Data    Tipo Mov           Valor           Saldo"
@PRow()+1,000 PSay __PrtThinLine()
